...
def test_schemes_and_scheme_detail(runner: CliRunner) -> None:
    ...
    assert (
        "table" in run(runner, "schemes").output.lower()
        or "vs-bytes" in run(runner, "schemes").output
    )
...
def test_sanitize_roundtrip_and_list(runner: CliRunner) -> None:
    ...
    staged = run(
        runner, "sanitize", "--text", text, "--stage", "strip_zero_width", "-e", "json"
    ).output
...
def test_verify_all_channels_pass(runner: CliRunner) -> None:
    ...
    results = json.loads(
        run(runner, "verify", "--scheme", "homoglyph", "--scheme", "tags", "--json").output
    )
...
def test_exit_codes_for_failure_modes(runner: CliRunner) -> None:
    ...
    assert (
        run(
            runner, "encode", "--cover", "tiny", "--payload", "x" * 3000, "--scheme", "homoglyph"
        ).exit_code
        == 5
    )
...
def test_report_goes_to_stderr_only(runner: CliRunner) -> None:
    result = run(
        runner, "encode", "--cover", "m" * 900, "--payload", "hi", "--scheme", "tags", "--report"
    )
    assert "scheme=tags" in result.output  # CliRunner mixes both streams
    result = runner.invoke(
        main,
        ["encode", "--cover", "m" * 900, "--payload", "hi", "--scheme", "tags"],
        input=None,

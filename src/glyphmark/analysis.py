...
CONFUSABLE_ALTS: dict[int, str] = {ord(alt): base for base, alt in (*HOMOGLYPH_PAIRS, *PUNCT_PAIRS)}
LATEX_SPACEISH = {
    0x00A0,
    0x1680,
    0x2000,
...
_FINDINGS: list[dict] = [
    ...
    dict(
        kind="fullwidth_forms",
        severity="low",
        weight=8,
        title="Fullwidth / compatibility look-alikes",
        why="U+FF01-U+FF5E twins of printable ASCII — a 1-bit channel NFKC removes in one pass.",
    ...
    dict(
        kind="combining_marks",
        severity="low",
        weight=8,
        title="Combining marks on unexpected bases",
        why="Stacking U+0300-U+036F onto letters/spaces is a low-stealth but real channel.",
        stage="strip_combining",
    ),
    dict(
        kind="default_ignorable",
        severity="medium",
        weight=12,
...
def detect(text: str, try_frames: bool = True) -> dict:
        ...
        findings.append(
            {
                ...
                "severity": meta["severity"],
                "count": len(mixed),
                "offsets": [m["offset"] for m in mixed[:40]],
                "why": meta["why"],
                "remove_with": meta["stage"],
                "tokens": mixed[:20],
            }
    ...
    if drift:
        findings.append(
            {
                "kind": "nfkc_drift",
                "title": "Text changes under NFKC normalization",
                "severity": "info",
                "count": drift,
                "offsets": [],
        ...
        findings.append(
            {
                ...
                "title": f"GlyphMark frame recovered via '{hit['scheme']}'",
                "severity": "high",
                "count": hit["payload_len"],
                "offsets": [],
...
SCHEME_STAGE = {
    "zw-binary": "strip_zero_width",
    "zw-octal": "strip_zero_width",
    "omni16": "strip_format",
    "tags": "strip_tags",
    "vs-bytes": "strip_variation_selectors",
    "bidi": "strip_bidi",
    "fillers": "strip_fillers",
    "homoglyph": "confusables_to_ascii",
    "fullwidth": "fullwidth_fold",
    "punct": "punct_fold",
    "spaces": "collapse_spaces",
    "ascii-ctrl": "strip_controls",
    "combining": "strip_combining",
}
...
def _recommendation(findings: list[dict], verdict: str) -> str:
    ...
    if verdict == "clean":
        return (
            "No covert-channel code points detected. Still normalize untrusted input "
            "(NFKC) before it reaches a model or a template."
        )
    return (
        "Run the sanitizer with these stages to neutralize everything found: "
        + ", ".join(stages)
        + ". For an intake pipeline use the full default pipeline."
    )
